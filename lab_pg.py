...
def find_first_available_estatus_row(
    values: list[list[str]], headers: list[str], fields_to_save: list[str]
) -> int:
    """Encuentra una fila libre aunque Columna 1 tenga fórmulas/IDs precargados."""

    content_columns = [
        column
        for column in fields_to_save
        if column != ID_COLUMN and get_header_position(headers, column) is not None
    ]
    if not content_columns:
        content_columns = [ID_COLUMN]

    column_indexes = []
    for column in content_columns:
        column_position = get_header_position(headers, column)
        if column_position is not None:
            column_indexes.append(column_position - 1)

    for row_number, row in enumerate(values[2:], start=3):
        row_has_content = any(
            column_index < len(row) and clean_cell(row[column_index]).strip()
...
def append_estatus_row(row_dict: dict[str, Any]) -> int:
    ...
    headers = values[1]
    target_row = find_first_available_estatus_row(
        values, headers, NEW_ORDER_ESTATUS_FIELDS
    )
