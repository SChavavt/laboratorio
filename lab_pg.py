...
def render_estefano_forms_review(can_edit: bool) -> None:
    ...
    if selected_link:
        ...
        st.caption(
            "Este botón no guarda ni cambia el STATUS. Solo prepara el link para que "
            "aparezca automáticamente en el campo 'O pegar link de archivos' del subtab "
            "🚚 Envío de documentos."
        )
        if st.button(
            "📋 Copiar link al formulario de Envío",
            disabled=not can_edit,
            help=(
                "Carga este link en memoria para usarlo en el campo 'O pegar link de archivos' "
                "cuando abras el subtab 🚚 Envío de documentos. Después debes guardar el cambio ahí."
            ),
        ):
            st.session_state["estefano_forms_selected_url"] = selected_link
            st.success(
                "Listo: el link quedó preparado. Abre '🚚 Envío de documentos', elige el pedido "
                "y guarda el cambio para asociarlo al caso."
            )
            st.rerun()
    else:
        st.warning(
