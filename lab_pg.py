...
ALERT_DETAIL_COLUMNS = [
    ...
    "HORAS_TRANSCURRIDAS",
    "HORAS_ATRASO",
    "ESTADO_ALERTA_VISUAL",
]


def safe_float(value: Any) -> float | None:
    """Convierte texto numérico a float sin romper la vista si viene vacío."""
...
def build_tiempos_alerts_df() -> pd.DataFrame:
    """Construye la tabla de tiempos con alertas visibles y datos del pedido."""

    tiempos_df = read_sheet_df(SHEET_TIEMPOS)
    if tiempos_df.empty:
        return pd.DataFrame(columns=ALERT_DETAIL_COLUMNS)
    ...
    tiempos_df["HORAS_ATRASO"] = tiempos_df.apply(
        lambda row: max(
            (safe_float(row.get("HORAS_TRANSCURRIDAS", "")) or 0)
            - (safe_float(row.get("TIEMPO_MAXIMO_HORAS", "")) or 0),
            0,
        )
        if clean_cell(row.get("ESTADO_ALERTA_VISUAL", "")) == "Atrasado"
        else 0,
        axis=1,
    )
    ...
    if not estatus_df.empty and ID_COLUMN in estatus_df.columns and ID_COLUMN in tiempos_df.columns:
        detail_columns = [
            column
            for column in [ID_COLUMN, "NOMBRE DOCTOR", "NOMBRE PACIENTE", "VENDEDOR"]
            if column in estatus_df.columns
        ]
        if len(detail_columns) > 1:
            detalles_df = estatus_df[detail_columns].drop_duplicates(subset=[ID_COLUMN])
            tiempos_df = tiempos_df.merge(detalles_df, on=ID_COLUMN, how="left")

    ordered_df = pd.concat([tiempos_df[active_mask], tiempos_df[~active_mask]], ignore_index=True)
    for column in ALERT_DETAIL_COLUMNS:
        if column not in ordered_df.columns:
            ordered_df[column] = ""
    return ordered_df


def render_alert_metrics(df: pd.DataFrame) -> None:
    """Muestra métricas compactas de alertas."""

    counts = df["ESTADO_ALERTA_VISUAL"].value_counts().to_dict() if not df.empty else {}
...
def render_global_alert_summary() -> None:
    """Muestra avisos de alertas fuera de los tabs para verlos al cargar la app."""

    alerts_df = build_tiempos_alerts_df()
    if alerts_df.empty:
        st.info("⏱️ No hay registros en TIEMPOS_APARATOS para calcular alertas.")
        return

    st.markdown("### 🚨 Resumen general de tiempos")
    render_alert_metrics(alerts_df)

    attention_df = alerts_df[
        alerts_df["ESTADO_ALERTA_VISUAL"].isin(["Atrasado", "Próximo a vencer"])
        & (alerts_df["REGISTRO_ACTIVO"] == "Sí")
    ].copy()
    if attention_df.empty:
        st.success("✅ No hay aparatos atrasados ni próximos a vencer en este momento.")
        return

    delayed_count = int((attention_df["ESTADO_ALERTA_VISUAL"] == "Atrasado").sum())
    near_due_count = int((attention_df["ESTADO_ALERTA_VISUAL"] == "Próximo a vencer").sum())
    st.warning(
        f"Hay {delayed_count} aparato(s) atrasado(s) y {near_due_count} próximo(s) a vencer. "
        "Revisa la tabla de abajo y actualiza el STATUS desde 📋 Seguimiento de pedidos."
    )
    st.dataframe(
        attention_df[ALERT_DETAIL_COLUMNS],
        use_container_width=True,
        hide_index=True,
        column_config=build_dataframe_column_config(attention_df[ALERT_DETAIL_COLUMNS]),
    )

def render_tiempos_tab() -> None:
    st.subheader("⏱️ Tiempos y Alertas")
    ordered_df = build_tiempos_alerts_df()
    ...
    st.caption(
        "Se muestran solo las columnas útiles para operación. "
        "FASE_ORDEN, STATUS_SIGUIENTE, RESPONSABLE y ESTADO_ALERTA se ocultan "
        "porque no se usan para decidir acciones; usa ESTADO_ALERTA_VISUAL."
    )

    active_alerts_df = ordered_df[
        (ordered_df["REGISTRO_ACTIVO"] == "Sí")
        & ordered_df["ESTADO_ALERTA_VISUAL"].isin(["Atrasado", "Próximo a vencer"])
    ].copy()
    if not active_alerts_df.empty:
        st.markdown("#### 🚨 Pedidos que requieren atención")
        st.dataframe(
            active_alerts_df[ALERT_DETAIL_COLUMNS],
            use_container_width=True,
            hide_index=True,
            column_config=build_dataframe_column_config(active_alerts_df[ALERT_DETAIL_COLUMNS]),
        )
        st.caption(
            "Para cambiar el siguiente STATUS o editar el pedido, usa la pestaña "
            "📋 Seguimiento de pedidos y busca la Columna 1 mostrada aquí."
        )

    st.markdown("#### 📋 Historial de tiempos")
    st.dataframe(
        ordered_df[ALERT_DETAIL_COLUMNS],
        use_container_width=True,
        hide_index=True,
        column_config=build_dataframe_column_config(ordered_df[ALERT_DETAIL_COLUMNS]),
    )

    st.markdown("#### 📊 Resumen")
    render_alert_metrics(ordered_df)
