...
USER_ALLOWED_TRANSITIONS = {
    ...
    "Lesly": {
        ...
        "EN SINTERIZADO Y HORNEADO": ["LISTO P/CONFECCIÓN"],
    },
    "Vero": {
        ...
        "LISTO P/EMPAQUETADO": ["GENERACIÓN DE GUÍA"],
    },
}
...
def get_case_commercial_payment_status(identifier: str) -> str:
    """Lee el estado comercial PAGO del pedido en ESTATUS APARATOS."""

    estatus_df = read_sheet_df(SHEET_ESTATUS)
    if (
        estatus_df.empty
        or ID_COLUMN not in estatus_df.columns
        or "PAGO" not in estatus_df.columns
    ):
        return ""
    matches = estatus_df[
        estatus_df[ID_COLUMN].apply(
            lambda value: clean_cell(value).strip() == clean_cell(identifier).strip()
        )
    ]
    if matches.empty:
        return ""
...
def can_advance_from_payment(identifier: str, current_status: str) -> tuple[bool, str]:
    ...
    if clean_cell(active_row.get("PUEDE_AVANZAR", "")).strip() != "Sí":
        if get_case_commercial_payment_status(identifier) == "TOTAL":
            now = app_now().strftime("%Y-%m-%d %H:%M:%S")
            update_active_tiempo_row(
                identifier,
                {
                    "PAGO_ESTADO": "Aprobado",
                    "PAGO_FECHA": clean_cell(active_row.get("PAGO_FECHA", "")).strip() or now,
                    "PAGO_VALIDADO_POR": clean_cell(
                        active_row.get("PAGO_VALIDADO_POR", "")
                    ).strip()
                    or "Sistema",
                    "PUEDE_AVANZAR": "Sí",
                    "MOTIVO_BLOQUEO": "",
