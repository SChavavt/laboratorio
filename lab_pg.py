...
def render_edit_field(column: str, value: Any, key: str) -> Any:
    ...
    if canonical_column in DATE_COLUMNS:
        ...
        if parsed_date is None and not text_value.strip():
            st.caption(f"{display_field_label(column)}: sin fecha guardada.")
            should_save_date = st.checkbox(
                f"Agregar {display_field_label(column)}", key=f"{key}_enabled"
            )
            if not should_save_date:
                return ""
    ...
    if canonical_column in DATETIME_TEXT_COLUMNS:
        ...
        if parsed_datetime is None and not text_value.strip():
            st.caption(f"{display_field_label(column)}: sin fecha/hora guardada.")
            should_save_datetime = st.checkbox(
                f"Agregar {display_field_label(column)}", key=f"{key}_enabled"
            )
            if not should_save_datetime:
                return ""
